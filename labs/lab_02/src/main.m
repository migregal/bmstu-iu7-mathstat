...
function process_mu (X, gamma, muhat)
    process_parameter(X, gamma, muhat, @normfit_mu, '$\hat\mu(\vec x_N)$', '$\hat\mu(\vec x_n)$', '$\underline\mu(\vec x_n)$', '$\overline\mu(\vec x_n)$');
    %process_parameter(X, gamma, muhat, @normfit_mu, '$\mu(N)$', '$\\mu(n)$', '$\mu_low(n)$', '$\mu_high(n)$');
endfunction

function process_s2 (X, gamma, S2)
    process_parameter(X, gamma, S2, @normfit_s2, '$\hat\sigma^2(\vec x_N)$', '$\hat\sigma^2(\vec x_n)$', '$\underline\sigma^2(\vec x_n)$', '$\overline\sigma^2(\vec x_n)$');
    %process_parameter(X, gamma, S2, @normfit_s2, '$\sigma^2(N)$', '$\sigma^2(n)$', '$\sigma^2_low(n)$', '$\sigma^2_high(n)$');
endfunction

% 1-2
[muhat, muci] = normfit_mu(X, 1 - gamma);
fprintf("Выборочное среднее: %f\n", muhat);
fprintf("Доверительный интервал уровня %.2f для мат. ожидания: [%f ; %f]\n", gamma, muci(1), muci(2));

[s2hat, s2ci] = normfit_s2(X, 1 - gamma);
fprintf("Исправленная выборочная дисперсия: %f\n", s2hat);
fprintf("Доверительный интервал уровня %.2f для дисперсии: [%f ; %f]\n", gamma, s2ci(1), s2ci(2));
